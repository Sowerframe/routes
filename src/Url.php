...
class Url
{
     ...
     * @param  array  $vars 参数
     */
    public function __construct(Route $routes, App $app, string $url = '', array $vars = [])
    {
        $this->routes = $routes;
        $this->app   = $app;
        $this->url   = $url;
    ...
    protected function parseDomain(string &$url, $domain): string
    {
        ...
        if (true === $domain) {
            // 自动判断域名
            $domain  = $request->host();
            $domains = $this->routes->getDomains();
    ...
    {
        if ($suffix) {
            $suffix = true === $suffix ? $this->routes->config('url_html_suffix') : $suffix;
    ...
    protected function parseUrl(string $url, &$domain): string
    {
        ...
        if (0 === strpos($url, '/')) {
            ...
            if ($this->routes->config('url_convert')) {
            ...
            if ($app && $this->app->config->get('app.apps_switch')) {
    ...
    protected function getRuleUrl(array $rule, array &$vars = [], $allowDomain = ''): array
    {
        ...
        foreach ($rule as $item) {
            ...
            $type = $this->routes->config('url_common_param');
    ...
    public function build()
    {
        ...
        if ($url) {
            ...
            $rule = $this->routes->getName($checkName, $checkDomain);

            if (empty($rule) && isset($info['query'])) {
                $rule = $this->routes->getName($url, $checkDomain);
                // 解析地址里面参数 合并到vars
        ...
        if (!empty($rule) && $match = $this->getRuleUrl($rule, $vars, $domain)) {
            ...
            if ($request->app() && $this->app->config->get('app.apps_switch') && !$this->app->http->isBindDomain()) {
                $url = $request->app() . '/' . $url;
            }
        } elseif (!empty($rule) && isset($name)) {
            throw new \InvalidArgumentException('routes name not exists:' . $name);
        } else {
            // 检测URL绑定
            $bind = $this->routes->getDomainBind($domain && is_string($domain) ? $domain : null);

            if ($bind && 0 === strpos($url, $bind)) {
                $url = substr($url, strlen($bind) + 1);
            } else {
                $binds = $this->routes->getBind();
        ...
        // 还原URL分隔符
        $depr = $this->routes->config('pathinfo_depr');
        ...
        if (!empty($vars)) {
            // 添加参数
            if ($this->routes->config('url_common_param')) {
