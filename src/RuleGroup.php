...
class RuleGroup extends Rule
{
    ...
    protected function checkMergeRuleRegex(Request $request, array &$rules, string $url, bool $completeMatch)
    {
        ...
        foreach ($rules as $key => $item) {
            if ($item instanceof RuleItem) {
                ...
                if (preg_match_all('/[' . $slash . ']?<?\w+\??>?/', $rule, $matches)) {
                    ...
                    $regex[$key] = $this->buildRuleRegex($rule, $matches[0], $pattern, $option, $complete, '_Sower_' . $key);
                    $items[$key] = $item;
                }
            }
        }
        ...
        try {
            ...
            throw new Exception('routes pattern error');
        }
        ...
        if ($result) {
            ...
            foreach ($match as $key => $val) {
                if (is_string($key) && '' !== $val) {
                    list($name, $pos) = explode('_Sower_', $key);
     ...
     * 注册MISS路由
     * @access public
     * @param  string|Closure $routes  路由地址
     * @param  string         $method 请求类型
     * @return RuleItem
     */
    public function miss($routes, string $method = '*') : RuleItem
    {
        // 创建路由规则实例
        $ruleItem = new RuleItem($this->router, $this, null, '', $routes, strtolower($method));
     ...
     * @return RuleItem
     */
    public function addRule(string $rule, $routes = null, string $method = '*'): RuleItem
    {
        // 读取路由标识
        if (is_string($routes)) {
            $name = $routes;
        ...
        // 创建路由规则实例
        $ruleItem = new RuleItem($this->router, $this, $name, $rule, $routes, $method);
