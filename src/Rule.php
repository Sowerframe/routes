...
use sower\Response;
use sower\Route;
use sower\routes\dispatch\Callback as CallbackDispatch;
use sower\routes\dispatch\Controller as ControllerDispatch;
use sower\routes\dispatch\Redirect as RedirectDispatch;
use sower\routes\dispatch\Response as ResponseDispatch;
use sower\routes\dispatch\View as ViewDispatch;
abstract class Rule
{
    /**
     * 路由标识
    ...
    {
        return $this->middleware('\sower\savant\Crossdomain', $header);
    }
    ...
    {
        return $this->middleware('\sower\savant\Formtoken', $token);
    }
    ...
    {
        return $this->middleware('\sower\savant\Cachepro', $cache);
    }
     ...
     * @param  mixed   $routes 路由地址
     * @param  string  $url URL地址
     * @param  array   $option 路由参数
     * @param  array   $matches 匹配的变量
     * @return Dispatch
     */
    public function parseRule(Request $request, string $rule, $routes, string $url, array $option = [], array $matches = []): Dispatch
    {
        if (is_string($routes) && isset($option['prefix'])) {
            // 路由地址前缀
            $routes = $option['prefix'] . $routes;
        }

        // 替换路由地址中的变量
        if (is_string($routes) && !empty($matches)) {
            $search = $replace = [];
        ...
        // 发起路由调度
        return $this->dispatch($request, $routes, $option);
    }
     ...
     * @return Dispatch
     */
    protected function dispatch(Request $request, $routes, array $option): Dispatch
    {
        if ($routes instanceof Dispatch) {
            $result = $routes;
        } elseif ($routes instanceof \Closure) {
            // 执行闭包
            $result = new CallbackDispatch($request, $this, $routes, $this->vars);
        } elseif ($routes instanceof Response) {
            $result = new ResponseDispatch($request, $this, $routes);
        } elseif (isset($option['view']) && false !== $option['view']) {
            $result = new ViewDispatch($request, $this, $routes, is_array($option['view']) ? $option['view'] : $this->vars);
        } elseif (!empty($option['redirect']) || 0 === strpos($routes, '/') || strpos($routes, '://')) {
            // 路由到重定向地址
            $result = new RedirectDispatch($request, $this, $routes, $this->vars, $option['status'] ?? 301);
        } elseif (false !== strpos($routes, '\\')) {
            // 路由到类的方法
            $result = $this->dispatchMethod($request, $routes);
        } else {
            // 路由到控制器/操作
            $result = $this->dispatchController($request, $routes);
        }
     ...
     */
    protected function dispatchMethod(Request $request, string $routes): CallbackDispatch
    {
        $path = $this->parseUrlPath($routes);

        $routes  = str_replace('/', '@', implode('/', $path));
        $method = strpos($routes, '@') ? explode('@', $routes) : $routes;
     ...
     */
    protected function dispatchController(Request $request, string $routes): ControllerDispatch
    {
        $path = $this->parseUrlPath($routes);
    ...
    {
        return ['name', 'rule', 'routes', 'method', 'vars', 'option', 'pattern'];
    }

    public function __wakeup()
    {
        $this->router = Container::pull('routes');
    }
